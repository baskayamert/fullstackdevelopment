...
@Injectable()
export class SearchTextParameterGuard implements CanActivate {
  ...
  canActivate(route: ActivatedRouteSnapshot, state: RouterStateSnapshot): Observable<boolean> {
    ...
    const searchTextParam = route.params['searchText'];
    ...
    return this.cityService.getCitiesBySearchText(searchTextParam).pipe(
      switchMap((res : ApiResponseModel<GetCityDto[]>) => {
        ...
        if (isValid) {
          ...
          this.router.navigate([`/cities/searchResult/${searchTextParam}/1`]); // Redirect to a different route for invalid parameters
          return of(false);
        }
      }),
      catchError(() => {
